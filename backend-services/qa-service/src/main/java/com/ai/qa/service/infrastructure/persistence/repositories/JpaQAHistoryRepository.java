...
public interface JpaQAHistoryRepository extends JpaRepository<QAHistoryPO, Long> {
     ...
     */
    @Query("SELECT q FROM QAHistoryPO q WHERE q.sessionId = :sessionId ORDER BY q.createTime DESC")
     ...
     */
    @Query("SELECT q FROM QAHistoryPO q WHERE q.userId = :userId AND q.sessionId = :sessionId ORDER BY q.createTime DESC")
